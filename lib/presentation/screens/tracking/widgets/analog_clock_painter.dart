...
class AnalogClockPainter extends CustomPainter {
  ...
  _paintPinHole(canvas, size, scaleFactor) {
    ...
    canvas.drawCircle(size.center(Offset.zero),
        handPinHoleSize * scaleFactor, midPointStrokePainter);
  }
  ...
  void _paintClockHands(Canvas canvas, Size size, double scaleFactor) {
    ...
    double longHandLength = r - (p * scaleFactor) - 6;
    double shortHandLength = r - (p + 46.0) * scaleFactor;
    ...
    canvas.drawLine(
        ...
        handPaint..color = hourHandColor..strokeWidth = 5);

    canvas.drawLine(
        size.center(_getHandOffset(minutes, handPinHoleSize * scaleFactor)),
        size.center(_getHandOffset(minutes, longHandLength - 8)),
        handPaint..color = minuteHandColor..strokeWidth = 5);
    if (showSecondHand) {
      canvas.drawLine(
          size.center(
              _getHandOffset(seconds, handPinHoleSize * scaleFactor)),
          size.center(_getHandOffset(seconds, longHandLength)),
          handPaint..color = secondHandColor..strokeWidth = 3);
    }
  }
